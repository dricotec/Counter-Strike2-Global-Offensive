...
function _Init(mockdata) {
    if (_m_bInitialized) {
        $.Msg("Versus already initialized.");
        return true;
    }

    _m_totalTime = Number(GameInterfaceAPI.GetSettingString("sv_warmup_to_freezetime_delay")) + 0.5;

    var contextId = $.GetContextPanel().id;

    if (contextId == "LayoutPanel_CSGOVersus" && mockdata) {
        _m_cP = $("#LayoutPanel_CSGOVersus");
        _m_mockData = mockdata;
        MockAdapter.SetMockData(_m_mockData);
    } else if (contextId == "Versus" && !mockdata) {
        _m_cP = $("#Versus");
    } else {
        $.Msg("Invalid context panel for Versus init: " + contextId);
        return false;
    }

    if (!_m_cP || !_m_cP.IsValid()) {
        $.Schedule(0.05, function() { _Init(mockdata); }); // try once after short delay
        return false;
    }

    if (!_SelectScene()) {
        $.Msg("Warning: could not select scene, using fallback.");
        _m_oScene = { numModels: 1, sceneFile: "resource/ui/econ/VersusCharactersMine.res" };
    }

    if (!_m_oScene || _m_oScene.numModels === undefined) {
        $.Msg("Error: _m_oScene is invalid.");
        return false;
    }

    _m_elContentRoot = _m_cP.FindChildTraverse("id-vs-content-container");
    if (!_m_elContentRoot || !_m_elContentRoot.IsValid()) {
        $.Msg("Error: id-vs-content-container not found!");
        return false;
    }
    ...
    if (_m_elPreviewPanel_MyTeam && _m_elPreviewPanel_MyTeam.IsValid()) {
        _m_elPreviewPanel_MyTeam.SetScene(
            _m_oScene.sceneFile || "resource/ui/econ/VersusCharactersMine.res",
   ...
   _m_sTeamName = 'CT'; 
    ...
    _m_elMyTeam.SetDialogVariable("teamname", _m_sTeamName);
	...
	function _SelectScene ()
	{
		...
		$.DispatchEvent( 'PlaySoundEffect', 'UIPanorama.TeamIntro', 'MOUSE' );
		if ( !_m_oScene )
			$.Msg('Pizdec5');
