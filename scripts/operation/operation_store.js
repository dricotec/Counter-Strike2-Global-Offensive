...
var OperationStore = ( function() 
{
	...
	var _FillOutStoreTiles = function ()
	{
		...
		var aRowTypes = [ 'isCharacterLootlist', 'set_train_2025', [ 'isWeaponLootlist', 'isWeaponsCase', 'isStickerLootlist', 'isGraffitiBox' ] ];
		var bHasTitle = false;
		...
		aRowTypes.forEach((type, index) => {
			...
			aRewards.forEach( ( reward, index ) =>
			{
				...
				else if( strSetName !== 'set_train_2025' && Array.isArray( type ) && type.indexOf( reward.containerType) !== -1  )
				{
					aRow.push( reward );
					bHasTitle = false;
				}
