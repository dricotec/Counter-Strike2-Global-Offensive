    ...
    if (m_isWorkshop) {
        ...
        if (m_gameModeSetting === 'premier') {
            UiToolkitAPI.ShowCustomLayoutPopupParameters(
                '',
                'file://{resources}/layout/popups/popup_premier_matchmaking.xml',
                '',
                'none'
            );
        }
        ...
        if (m_gameModeSetting !== 'premier' && GameModeFlags.DoesModeUseFlags(_RealGameMode()) && !m_gameModeFlags[m_serverSetting + _RealGameMode()]) {
            btnStartSearch.RemoveClass('pressed');
...
function StartSearchFake() // this is fake matchmaking which i primarily used for debugging the styles of the match accept popups.. nothing special, most of the magic is in the popup_accept_match_fake.js script.
{
    ...
    function _IsGameModeAvailable(serverType, gameMode) {
        ...
        if (_IsValveOfficialServer(serverType) &&
            LobbyAPI.BIsHost()) {
            if (gameMode === 'premier') {
                isAvailable = (MyPersonaAPI.HasPrestige() || MyPersonaAPI.GetCurrentLevel() >= 10);
            }
...
function _ShowCancelSearchButton(isSearching, isHost) {
    ...
    function _UpdatePrimeBtn ( isSearching, isHost ) // fuck you valve for your junk code that needed fixing for the stupid prime button. before it was enable to disable prime, disable to enable prime. fuck you.
    {
    ...
    function _IsPrimeChecked () // just checks the prime button when values are received from the convar. also valve your code is the biggest piece of shit.
    {
...
$.OnGameModeFlagsBtnClicked = function (resumeMatchmakingHandle = '') {
    ...
    function OnPressOfficialServers() { // bot panel was fixed here by d3gk.. thanks to him bot panel hides when you go in official mm and unhides when you go in workshop and practice with bots
        m_isWorkshop = false;
