...
var _OnShowMainMenu = function() {
  $.DispatchEvent('PlayMainMenuMusic', true, true);
	...
	{	
		...
		var elMainMenuVanityInfo = $.CreatePanel( 'Panel', $.FindChildInContext( '#MainMenuVanityInfo' ), 'JsMainMenuVanityInfo' );
		elLeftColumn.BLoadLayout( 'file://{resources}/layout/vanity_player_info.xml', false, false );
	...
	var _HideMainMenuNewsPanel = function()
	{
		...
		var elMainMenuVanityInfo = $.FindChildInContext( '#MainMenuVanityInfo' );
		elMainMenuVanityInfo.SetHasClass( 'news-panel--hide-news-panel', true );
	...
	var _ShowNewsAndStore = function ()
	{
		...
		var elPanel = $.FindChildInContext( '#MainMenuVanityInfo' );
		elPanel.SetHasClass( 'hidden', false );
	...
	var _InitVanity = function( id, isLocalPlayer, settings )
	{
		...
		if(!isLocalPlayer) {
			...
			oSettings.team = info[0];
			oSettings.loadoutSlot = info[3];
			oSettings.charItemId = info[1];
			oSettings.weaponItemId = info[4];
			oSettings.glovesItemId = info[2];
		}
	...
	var _UpdateLobbyVanity = function( players )
	{
		for (let i = 0; i < 5; i++) {
			if(players[i] === undefined) {
				...
				var vanityPanel = $( '#JsMainmenu_Vanity_'+i );
				if ( !vanityPanel.BHasClass( 'hidden' ) ) {
					vanityPanel.AddClass( 'hidden' );
				}
				continue;
			}
			//UiToolkitAPI.ShowGenericPopupOk( 'cstring title', JSON.stringify(players[i]), '', function(){} );
			_InitVanity(players[i].playeridx, players[i].isLocalPlayer, players[i]);
        }
	};
...
var _PauseMainMenuCharacter = function() { // pauses your agent when changing background or disconnecting from a server.
    var vanityPanel = $('#JsMainmenu_Vanity_0');
	var vanityPanel_1 = $('#JsMainmenu_Vanity_1');
	var vanityPanel_2 = $('#JsMainmenu_Vanity_2');
	var vanityPanel_3 = $('#JsMainmenu_Vanity_3');
	var vanityPanel_4 = $('#JsMainmenu_Vanity_4');
